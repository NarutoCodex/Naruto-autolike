...
const User = mongoose.model('User', new mongoose.Schema({
    ...
    activePlans: [{
        uid: String,
        playerName: { type: String, default: "N/A" },
        days: Number,
        expiryDate: Date,
        totalLikes: { type: Number, default: 0 },
        likeBefore: { type: Number, default: 0 },
        likeAfter: { type: Number, default: 0 }
    }]
}));
...
// --- REJECT REQUEST (SIRF YE ADD KIYA HAI) ---
app.post('/api/admin/reject-balance', async (req, res) => {
    if (req.body.adminKey !== process.env.ADMIN_KEY) return res.status(403).send("No");
    const user = await User.findById(req.body.id);
    user.pendingBalance = 0; user.status = "new"; user.redeemCode = "";
    await user.save();
    res.json({ success: true });
});
            ...
            for (let plan of user.activePlans) {
                if (new Date(plan.expiryDate) > new Date()) {
                    try {
                        ...
                        if (resp.status === 200) { 
                            plan.likeBefore = plan.likeAfter || 0;
                            plan.totalLikes += 100; 
                            plan.likeAfter = plan.likeBefore + 100;
                            count++; 
                        }
